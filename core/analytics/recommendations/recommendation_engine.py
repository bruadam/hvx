...
class RecommendationEngine:
    ...
    def generate_recommendations(
        ...
        room_data: pd.DataFrame | None = None,
        climate_correlations: dict[str, Any] | None = None,
        ...
        Args:
            ...
            room_data: Optional room time series data for correlation analysis
            climate_correlations: Optional pre-computed climate correlations
        ...
        if self._has_temperature_violations(room_analysis):
            temp_recs = self._generate_temperature_recommendations(
                room_analysis, climate_data, room_data
            )
    ...
    def _generate_temperature_recommendations(
        ...
        climate_correlations = {}
        if climate_data is not None and room_data is not None and "temperature" in room_data.columns:
            # Perform correlation analysis between indoor temperature and climate factors
            try:
                indoor_temp = room_data["temperature"].dropna()

                # Correlate with outdoor temperature
                if "outdoor_temp" in climate_data.columns:
                    result = self.correlator.correlate_with_climate(
                        indoor_temp, climate_data, "outdoor_temp"
                    )
                    if result:
                        climate_correlations["outdoor_temp"] = result

                # Correlate with solar radiation
                if "solar_radiation" in climate_data.columns or "radiation" in climate_data.columns:
                    rad_col = "solar_radiation" if "solar_radiation" in climate_data.columns else "radiation"
                    result = self.correlator.correlate_with_climate(
                        indoor_temp, climate_data, rad_col
                    )
                    if result:
                        climate_correlations["solar_radiation"] = result

                # Correlate with wind speed
                if "wind_speed" in climate_data.columns:
                    result = self.correlator.correlate_with_climate(
                        indoor_temp, climate_data, "wind_speed"
                    )
                    if result:
                        climate_correlations["wind_speed"] = result
            except Exception as e:
                # If correlation fails, continue without climate data
                print(f"Warning: Climate correlation failed: {e}")

        # Determine recommendation based on pattern
        if "above" in worst_test_id.lower() or "overheat" in worst_test_id.lower() or "summer" in worst_test_id.lower():
            # Overheating issue
        ...
        elif "below" in worst_test_id.lower() or "cold" in worst_test_id.lower() or "winter" in worst_test_id.lower():
            # Underheating issue
