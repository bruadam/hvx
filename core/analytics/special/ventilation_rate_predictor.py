...
NOT VALIDATED YET - experimental feature.
"""
...
class VentilationRatePredictor:
    ...
    def predict_ventilation_rate(
        ...
        """
        Predict ventilation rate (wrapper for compatibility).

        Args:
            co2_series: CO2 time series
            occupancy: Number of occupants (not used in decay method)
            volume: Room volume in m³ (not used in decay method)

        Returns:
            Dictionary with estimated_ach, estimated_l_s, and confidence
        """
        result = self.predict_from_multiple_periods(co2_series)
        ...
        # Calculate L/s if volume provided
        estimated_l_s = None
        if volume is not None and volume > 0:
            # ACH = (L/s × 3600) / (volume × 1000)
            # L/s = (ACH × volume × 1000) / 3600
            estimated_l_s = (result.ach * volume * 1000) / 3600
        ...
        return {
            "estimated_ach": result.ach,
            "estimated_l_s": round(estimated_l_s, 1) if estimated_l_s else None,
            "confidence": confidence,
            "category": result.ventilation_category,
