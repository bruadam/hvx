...
class IEQInteractiveWorkflow:
    ...
    def __init__(self, auto_mode: bool = False, verbose: bool = False):
        ...
        self.building_analysis: BuildingAnalysis | None = None
    ...
    def _step_load_data(self) -> bool:
        ...
        try:
            with Progress(
                ...
                progress.add_task("Loading data from directory...", total=None)
    ...
    def _step_run_analysis(self) -> bool:
        ...
        if self.building_analysis:
            ...
            self.progress.complete_step(4, summary)
            console.print(create_success_panel("Analysis complete!", summary))
    ...
    def _get_tests_for_standards(self, standard_ids: list[str]) -> list[dict]:
        ...
        for standard_id in standard_ids:
            ...
            if test_dir.exists():
                ...
                yaml_files = list({f for f in yaml_files if f.is_file() and not f.name.startswith('.')})
