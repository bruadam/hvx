...
class ParameterThreshold:
    ...
    Categories are numbered 1-4:
    - Category 1: Best performance
    - Category 2: Good performance
    - Category 3: Moderate performance
    - Category 4: Poor performance / special condition
    """

    category_1_min: float | None = None
    category_1_max: float | None = None
    category_2_min: float | None = None
    category_2_max: float | None = None
    category_3_min: float | None = None
    category_3_max: float | None = None
    category_4_condition: str | None = None  # Special condition for category 4
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ParameterThreshold":
        ...
        if not data:
            return cls()
        return cls(
            category_1_min=data.get("category_1", {}).get("min"),
            category_1_max=data.get("category_1", {}).get("max"),
            category_2_min=data.get("category_2", {}).get("min"),
            category_2_max=data.get("category_2", {}).get("max"),
            category_3_min=data.get("category_3", {}).get("min"),
            category_3_max=data.get("category_3", {}).get("max"),
            category_4_condition=data.get("category_4", {}).get("condition"),
        )
...
class TAILParameterConfig:
    ...
    percentile: float | None = None  # For CO2 95th percentile
...
class TAILConfig:
    ...
    def get_supported_parameters(
        ...
        try:
            ...
            raise ValueError(f"Failed to get supported parameters: {e}") from e
