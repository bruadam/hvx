...
from core.enums.spatial import BuildingType
from core.schedules import PROFILE_DEFINITIONS, get_opening_profile_for_building_type
...
class OpeningHoursFilter:
    ...
    def __init__(
        ...
        if opening_hours:
            ...
            profile = get_opening_profile_for_building_type(building_type.value)
            weekday_map = PROFILE_DEFINITIONS.get(profile) or PROFILE_DEFINITIONS[next(iter(PROFILE_DEFINITIONS))]
            example_periods = weekday_map.get(0) or next(iter(weekday_map.values()))
            start_time, end_time = example_periods[0]
