...
@dataclass
class TimeRange:
    """Simple time range used to filter time series."""

    start: datetime
    end: datetime

    def normalize(self) -> "TimeRange":
        """Ensure start <= end."""
...
class TimeFilter:
    ...
    def filter_by_time_range(df: pd.DataFrame, time_range: TimeRange) -> pd.DataFrame:
        ...
        normalized = time_range.normalize()
        # Ensure normalized.start and normalized.end are pandas.Timestamp for comparison
    ...
    def filter_by_date_list(df: pd.DataFrame, dates: list[datetime]) -> pd.DataFrame:
        ...
        mask = pd.Series(df.index.date, index=df.index).isin(date_set)
    ...
    def exclude_holidays(
        ...
        mask = ~pd.Series(df.index.date, index=df.index).isin(holiday_dates)
