...
[bold]Understanding Results:[/bold]
  ...
  • Severity - CRITICAL > HIGH > MEDIUM > LOW > INFO
    ...
    def _show_portfolio_recommendations(self, buildings: List[BuildingAnalysis]):
        ...
        from src.core.analytics.smart_recommendations_service import generate_building_recommendations_report
        from src.core.models import RoomAnalysis
    ...
    def _evaluate_solar_shading_impact(self, report: dict, room_analyses: dict):
        """Evaluate the impact of implementing solar shading recommendations with detailed calculations."""
        console.clear()
        ...
        total_overheating_hours = 0
        total_summer_hours = 0
        outdoor_temp_correlations = []
        solar_correlations = []
        summer_compliance_rates = []

        for item in solar_shading_rooms:
            room = item['room']
            rec = item['recommendation']

            # Extract summer temperature data
            for test_result in room.test_results:
                if 'temp' in test_result.test_name.lower() and 'summer' in test_result.test_name.lower():
                    violations = len(test_result.violations)
                    total_overheating_hours += violations / 60  # Assume minute data
                    total_summer_hours += test_result.total_points / 60
                    summer_compliance_rates.append(test_result.compliance_rate)

            # Extract correlation data from metadata
            if hasattr(rec, 'metadata') and rec.metadata:
                if 'outdoor_temp_correlation' in rec.metadata:
                    outdoor_temp_correlations.append(rec.metadata['outdoor_temp_correlation'])
                if 'solar_radiation_correlation' in rec.metadata:
                    solar_correlations.append(rec.metadata['solar_radiation_correlation'])

        # Calculate aggregated metrics
        avg_summer_compliance = sum(summer_compliance_rates) / len(summer_compliance_rates) if summer_compliance_rates else 0
        avg_outdoor_corr = sum(outdoor_temp_correlations) / len(outdoor_temp_correlations) if outdoor_temp_correlations else 0
        avg_solar_corr = sum(solar_correlations) / len(solar_correlations) if solar_correlations else 0

        # Display detailed calculation metrics
        table = Table(title="📊 Solar Shading Impact Calculations", box=box.ROUNDED)
        table.add_column("Metric", style="cyan bold", width=40)
        table.add_column("Value", justify="right", width=20)
        table.add_column("Analysis", style="green", width=35)

        table.add_row("Total Overheating Hours", f"{total_overheating_hours:.1f}h", "")
        table.add_row("Summer Hours Analyzed", f"{total_summer_hours:.1f}h", "")
        table.add_row("Current Summer Compliance", f"{avg_summer_compliance:.1f}%", "Target: >95%")
        table.add_row("", "", "")

        # Correlation analysis
        table.add_row("[bold]Weather Correlations[/bold]", "", "")
        if outdoor_temp_correlations:
            corr_strength = "Strong" if abs(avg_outdoor_corr) > 0.5 else "Moderate" if abs(avg_outdoor_corr) > 0.3 else "Weak"
            table.add_row("  Outdoor Temperature", f"{avg_outdoor_corr:+.3f}", f"{corr_strength} correlation")

        if solar_correlations:
            corr_strength = "Strong" if abs(avg_solar_corr) > 0.5 else "Moderate" if abs(avg_solar_corr) > 0.3 else "Weak"
            table.add_row("  Solar Radiation", f"{avg_solar_corr:+.3f}", f"{corr_strength} correlation")

        table.add_row("", "", "")

        # Impact estimation based on correlations
        if avg_solar_corr > 0.5 or avg_outdoor_corr > 0.5:
            # Strong correlation = high impact potential
            estimated_improvement = min((100 - avg_summer_compliance) * 0.6, 40)
            table.add_row("Estimated Compliance Gain", f"+{estimated_improvement:.1f}%", "High confidence (strong correlation)")
        elif avg_solar_corr > 0.3 or avg_outdoor_corr > 0.3:
            # Moderate correlation = moderate impact
            estimated_improvement = min((100 - avg_summer_compliance) * 0.4, 25)
            table.add_row("Estimated Compliance Gain", f"+{estimated_improvement:.1f}%", "Moderate confidence")
        else:
            table.add_row("Estimated Compliance Gain", "Low (<10%)", "Weak correlation - limited impact")

        # Hours reduction potential
        if total_overheating_hours > 0:
            hours_reduction = total_overheating_hours * 0.5 if avg_solar_corr > 0.5 else total_overheating_hours * 0.3
            table.add_row("Overheating Hours Reduction", f"-{hours_reduction:.1f}h", f"{(hours_reduction/total_overheating_hours)*100:.0f}% reduction")

        console.print(table)
        console.print()

        # Show correlation interpretation
        console.print("[bold]Correlation Interpretation:[/bold]\n")
        if avg_solar_corr > 0.5:
            console.print("  [green]✓[/green] Strong solar radiation correlation indicates solar shading will be highly effective")
        elif avg_solar_corr > 0.3:
            console.print("  [yellow]~[/yellow] Moderate solar correlation - shading will help but other factors also contribute")
        else:
            console.print("  [red]✗[/red] Weak solar correlation - overheating may be driven by other factors (HVAC, insulation)")

        if avg_outdoor_corr > 0.5:
            console.print("  [yellow]![/yellow] Strong outdoor temperature correlation - consider insulation improvements too")
        console.print()

        # Show detailed analysis for high-priority rooms
        console.print("[bold]Calculation Details by Room:[/bold]\n")
        for i, item in enumerate(solar_shading_rooms[:5], 1):
            room = item['room']
            rec = item['recommendation']

            console.print(f"[cyan]{i}. {room.room_name}[/cyan]")

            # Extract room metrics
            room_compliance = 0
            for test_result in room.test_results:
                if 'temp' in test_result.test_name.lower() and 'summer' in test_result.test_name.lower():
                    room_compliance = test_result.compliance_rate

            console.print(f"   • Summer Compliance: {room_compliance:.1f}%")
            console.print(f"   • Priority: {rec.priority}")
            console.print(f"   • Rationale: {rec.description[:75]}...")
            console.print()

        if len(solar_shading_rooms) > 5:
            console.print(f"[dim]... and {len(solar_shading_rooms) - 5} more rooms[/dim]\n")
...
[bold]Cost-Benefit Analysis:[/bold]
  ...
  • Co-benefits: Reduced glare, lower cooling energy consumption
    ...
    def _evaluate_insulation_impact(self, report: dict, room_analyses: dict):
        """Evaluate the impact of implementing insulation improvements with detailed calculations."""
        console.clear()
        ...
        total_cold_hours = 0
        total_hot_hours = 0
        total_winter_hours = 0
        outdoor_temp_correlations = []
        winter_compliance_rates = []
        summer_compliance_rates = []

        for item in insulation_rooms:
            room = item['room']
            rec = item['recommendation']
            ...
            for test_result in room.test_results:
                ...
                if 'temp' in test_name and 'heating_season' in test_name:
                    violations = len(test_result.violations)
                    total_cold_hours += violations / 60
                    total_winter_hours += test_result.total_points / 60
                    winter_compliance_rates.append(test_result.compliance_rate)

                if 'temp' in test_name and 'summer' in test_name:
                    total_hot_hours += len(test_result.violations) / 60
                    summer_compliance_rates.append(test_result.compliance_rate)
        ...
        # Calculate metrics
        avg_winter_compliance = sum(winter_compliance_rates) / len(winter_compliance_rates) if winter_compliance_rates else 0
        avg_summer_compliance = sum(summer_compliance_rates) / len(summer_compliance_rates) if summer_compliance_rates else 0
        avg_outdoor_corr = sum(outdoor_temp_correlations) / len(outdoor_temp_correlations) if outdoor_temp_correlations else 0

        # Determine issue type
        issue_type = "both" if total_cold_hours > 50 and total_hot_hours > 50 else "heat_loss" if total_cold_hours > 50 else "heat_gain"

        # Display calculations
        table = Table(title="📊 Insulation Impact Calculations", box=box.ROUNDED)
        table.add_column("Metric", style="cyan bold", width=40)
        table.add_column("Value", justify="right", width=20)
        table.add_column("Analysis", style="green", width=35)

        table.add_row("[bold]Issue Type[/bold]", issue_type.replace('_', ' ').title(), "")
        table.add_row("", "", "")

        table.add_row("Cold Hours (Winter)", f"{total_cold_hours:.1f}h", "Below comfort range")
        table.add_row("Hot Hours (Summer)", f"{total_hot_hours:.1f}h", "Above comfort range")
        table.add_row("Winter Hours Analyzed", f"{total_winter_hours:.1f}h", "")
        table.add_row("", "", "")

        table.add_row("Winter Compliance", f"{avg_winter_compliance:.1f}%", "Target: >95%")
        if summer_compliance_rates:
            table.add_row("Summer Compliance", f"{avg_summer_compliance:.1f}%", "Target: >95%")
        table.add_row("", "", "")

        # Correlation analysis
        if outdoor_temp_correlations:
            corr_strength = "Strong" if abs(avg_outdoor_corr) > 0.6 else "Moderate" if abs(avg_outdoor_corr) > 0.4 else "Weak"
            table.add_row("Outdoor Temp Correlation", f"{avg_outdoor_corr:+.3f}", f"{corr_strength} - indicates heat transfer")

        table.add_row("", "", "")

        # Impact estimation
        if issue_type == "both":
            winter_gain = min((100 - avg_winter_compliance) * 0.5, 30)
            summer_gain = min((100 - avg_summer_compliance) * 0.4, 25)
            table.add_row("Est. Winter Compliance Gain", f"+{winter_gain:.1f}%", "From reduced heat loss")
            table.add_row("Est. Summer Compliance Gain", f"+{summer_gain:.1f}%", "From reduced heat gain")
        elif issue_type == "heat_loss":
            winter_gain = min((100 - avg_winter_compliance) * 0.6, 35)
            table.add_row("Est. Winter Compliance Gain", f"+{winter_gain:.1f}%", "From improved insulation")
        elif issue_type == "heat_gain":
            summer_gain = min((100 - avg_summer_compliance) * 0.5, 30)
            table.add_row("Est. Summer Compliance Gain", f"+{summer_gain:.1f}%", "From reduced solar heat gain")

        # Energy savings potential
        if total_cold_hours > 100:
            heating_reduction = total_cold_hours * 0.4
            table.add_row("Heating Hours Reduction", f"-{heating_reduction:.1f}h", f"{(heating_reduction/total_cold_hours)*100:.0f}% less heating needed")
        ...
        if abs(avg_outdoor_corr) > 0.6:
            console.print("  [green]✓[/green] Strong outdoor correlation confirms poor thermal envelope")
            console.print("     → Indoor temps closely track outdoor changes = heat transfer through building fabric")
        elif abs(avg_outdoor_corr) > 0.4:
            console.print("  [yellow]~[/yellow] Moderate correlation - insulation will help but HVAC also plays a role")
        else:
            console.print("  [red]✗[/red] Weak correlation - issues may be HVAC-related rather than insulation")

        console.print()

        # Room details
        console.print("[bold]Calculation Details by Room:[/bold]\n")
        for i, item in enumerate(insulation_rooms[:5], 1):
            room = item['room']
            rec = item['recommendation']
            ...
            room_winter_comp = 0
            for test_result in room.test_results:
                if 'temp' in test_result.test_name.lower() and 'heating_season' in test_result.test_name.lower():
                    room_winter_comp = test_result.compliance_rate

            console.print(f"   • Winter Compliance: {room_winter_comp:.1f}%")
            console.print(f"   • Priority: {rec.priority}")
            console.print(f"   • Recommendation: {rec.description[:75]}...")
            console.print()

        if len(insulation_rooms) > 5:
            console.print(f"[dim]... and {len(insulation_rooms) - 5} more rooms[/dim]\n")

        Prompt.ask("\n[bold]Press Enter to continue[/bold]", default="")
    
    def _evaluate_ventilation_impact(self, report: dict, room_analyses: dict):
        """Evaluate the impact of implementing ventilation improvements with detailed calculations."""
        console.clear()
        ...
        # Aggregate calculations across all rooms
        total_co2_violations = 0
        total_hours_analyzed = 0
        avg_co2_during_violations = []
        avg_co2_rise_rates = []

        for item in ventilation_rooms:
            room = item['room']
            rec = item['recommendation']

            # Extract CO2 data from test results
            for test_result in room.test_results:
                if 'co2' in test_result.test_name.lower():
                    total_co2_violations += len(test_result.violations)
                    total_hours_analyzed += test_result.total_points / 60  # Assume minutes to hours

                    # Get average CO2 from statistics
                    if 'avg' in test_result.statistics:
                        avg_co2_during_violations.append(test_result.statistics['avg'])

        # Calculate aggregated metrics
        avg_co2 = sum(avg_co2_during_violations) / len(avg_co2_during_violations) if avg_co2_during_violations else 0
        violation_rate = (total_co2_violations / (total_hours_analyzed * 60)) * 100 if total_hours_analyzed > 0 else 0

        # Display detailed calculation metrics
        table = Table(title="📊 Ventilation Impact Calculations", box=box.ROUNDED)
        table.add_column("Metric", style="cyan bold", width=40)
        table.add_column("Value", justify="right", width=20)
        table.add_column("Impact", style="green", width=30)

        table.add_row("Total CO2 Violations", f"{total_co2_violations:,}", "")
        table.add_row("Hours Analyzed", f"{total_hours_analyzed:.1f}h", "")
        table.add_row("Violation Rate", f"{violation_rate:.2f}%", "")
        table.add_row("Avg CO2 During Violations", f"{avg_co2:.0f} ppm", "Target: <1000 ppm")
        table.add_row("", "", "")

        # Estimated improvements
        if avg_co2 > 1000:
            reduction_potential = ((avg_co2 - 800) / avg_co2) * 100
            table.add_row("Potential CO2 Reduction", f"{reduction_potential:.1f}%", "With proper ventilation")

            compliance_improvement = min(violation_rate * 0.7, 40)  # Estimate 70% of violations resolved
            table.add_row("Estimated Compliance Gain", f"+{compliance_improvement:.1f}%", "Approximate improvement")
        ...
        # Show detailed analysis for high-priority rooms
        console.print("[bold]Calculation Details by Room:[/bold]\n")
        for i, item in enumerate(ventilation_rooms[:5], 1):  # Show top 5
            room = item['room']
            rec = item['recommendation']

            console.print(f"[cyan]{i}. {room.room_name}[/cyan]")

            # Extract room-specific metrics
            room_co2_avg = 0
            room_violations = 0
            for test_result in room.test_results:
                if 'co2' in test_result.test_name.lower():
                    room_violations = len(test_result.violations)
                    room_co2_avg = test_result.statistics.get('avg', 0)

            console.print(f"   • Average CO2: {room_co2_avg:.0f} ppm")
            console.print(f"   • Violations: {room_violations}")
            console.print(f"   • Priority: {rec.priority}")
            console.print(f"   • Recommendation: {rec.description[:80]}...")
            console.print()

        if len(ventilation_rooms) > 5:
            console.print(f"[dim]... and {len(ventilation_rooms) - 5} more rooms[/dim]\n")
