...
the complete data analysis workflow following the INSTRUCTIONS.md:
...
class InteractiveWorkflow:
    ...
    def _show_welcome(self):
        ...
        welcome_panel = Panel.fit(
            "[bold cyan]HVX - IEQ Analytics Workflow[/bold cyan]\n\n"
            "[bold]Interactive workflow steps:[/bold]\n"
            ...
            "[dim]Type 'help' for assistance or 'quit' to exit at any time[/dim]",
            border_style="cyan"
        )
    ...
    def _main_workflow(self):
        """Main workflow orchestration following INSTRUCTIONS.md."""
        try:
    ...
    def _step_load_data(self) -> bool:
        ...
        if self.data_directory:
            ...
            data_dir = self._prompt_with_help(
                "[bold]Enter the path to your data directory[/bold]",
                default="data/samples/sample-extensive-data",
                help_text="Your data directory should contain building folders with sensor CSV files."
            )
    ...
    def _show_dataset_summary(self):
        ...
        table = Table(box=box.ROUNDED, show_header=False, title="📊 Dataset Summary")
    ...
    def _step_assign_room_types(self) -> bool:
        ...
        # Show summary
        console.print(f"\n[yellow]⚠ Found {len(rooms_without_type)} room(s) without assigned types[/yellow]\n")
        ...
        choice = Prompt.ask("[bold]Choose option[/bold]", choices=["1", "2", "3", "4"], default="1")
    ...
    def _explore_analytics_interactively(self) -> bool:
        """Display analytics with interactive filtering options."""
    ...
    def _show_error_specific_help(self, error_context: str):
        ...
        help_messages = {
            ...
            "loading_error": "Check that CSV files are properly formatted with timestamp and value columns.",
...
def launch_interactive_workflow(
    ...
    Args:
        data_directory: Path to data directory (optional)
        auto_mode: If True, use default values for all prompts without asking
    """
