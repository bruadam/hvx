...
from src.core.analytics.ieq.AnalysisEngine import AnalysisEngine
from src.core.analytics.ieq.types import Method
from src.core.analytics.ieq.FilterProcessor import FilterProcessor
...
class HierarchicalAnalysisService:
    ...
    def __init__(
        ...
        Args:
            config_path: Path to tests.yaml configuration file (optional if config_dict provided)
    ...
    def _analyze_room(self, room: Room, building: Building) -> RoomAnalysis:
        ...
        # Assign EN16798-1 category (if applicable)
        self._assign_en16798_category(analysis)
    ...
    def _run_room_tests(self, room: Room, building: Building) -> Dict[str, TestResult]:
        """Run configured tests on room data, filtered by room type."""
        test_results = {}

        tests_config = self.config.get('analytics', {})

        # Get room type value for filtering
        room_type = room.room_type.value if room.room_type else None

        for test_name, test_config in tests_config.items():
            try:
                # Filter test by room type
                if not self._test_applies_to_room(test_config, room_type):
                    continue
    ...
    def _test_applies_to_room(self, test_config: Dict[str, Any], room_type: Optional[str]) -> bool:
        """
        Check if a test applies to a specific room type.

        Args:
            test_config: Test configuration dictionary
            room_type: Room type value (e.g., 'office', 'classroom') or None

        Returns:
            True if test applies to room type
        """
        room_types = test_config.get('room_types', [])

        # Empty list or missing means applies to all room types
        if not room_types:
            return True

        # If room has no type, apply all tests
        if not room_type:
            return True

        # Check if room type is in the allowed list
    ...
    def _assign_en16798_category(self, analysis: RoomAnalysis) -> None:
        """
        Assign EN16798-1 category based on compliance with category tests.

        A room is assigned to a category if it meets the requirements for that category
        >= 95% of the time for both temperature and CO2 (where applicable).

        Categories:
        - Cat I: High level of expectation (most stringent)
        - Cat II: Normal level of expectation
        - Cat III: Moderate level of expectation
        - Cat IV: Values outside other categories (acceptable for limited time)
        """
        # Extract EN16798-1 test results by category
        category_compliance: Dict[str, List[float]] = {
            "Cat I": [],
            "Cat II": [],
            "Cat III": [],
            "Cat IV": []
        ...
        for test_name, result in analysis.test_results.items():
            # Check if this is an EN16798-1 test
            if 'cat_i' in test_name.lower() or 'category_i' in test_name.lower():
                category_compliance["Cat I"].append(result.compliance_rate)
            elif 'cat_ii' in test_name.lower() or 'category_ii' in test_name.lower():
                category_compliance["Cat II"].append(result.compliance_rate)
            elif 'cat_iii' in test_name.lower() or 'category_iii' in test_name.lower():
                category_compliance["Cat III"].append(result.compliance_rate)
            elif 'cat_iv' in test_name.lower() or 'category_iv' in test_name.lower():
                category_compliance["Cat IV"].append(result.compliance_rate)

        # Determine category assignment (95% threshold)
        # Check from most stringent to least stringent
        threshold = 95.0

        for category in ["Cat I", "Cat II", "Cat III", "Cat IV"]:
            compliance_rates = category_compliance[category]

            if not compliance_rates:
                continue

            # All tests for this category must meet 95% compliance
            avg_compliance = sum(compliance_rates) / len(compliance_rates)
            min_compliance = min(compliance_rates)

            if min_compliance >= threshold:
                # Assign this category
                analysis.en16798_category = category
                analysis.en16798_category_confidence = avg_compliance
                logger.debug(f"Room {analysis.room_name} assigned to {category} "
                           f"(avg: {avg_compliance:.1f}%, min: {min_compliance:.1f}%)")
                return

        # If no category meets threshold, assign based on best fit
        best_category = None
        best_score = 0.0

        for category, compliance_rates in category_compliance.items():
            if not compliance_rates:
                continue

            avg_compliance = sum(compliance_rates) / len(compliance_rates)
            if avg_compliance > best_score:
                best_score = avg_compliance
                best_category = category

        if best_category and best_score > 0:
            analysis.en16798_category = best_category
            analysis.en16798_category_confidence = best_score
            logger.debug(f"Room {analysis.room_name} partially meets {best_category} "
                       f"({best_score:.1f}% < {threshold}% threshold)")
