...
from .StandardRegistry import get_global_registry
from .library.evaluators import (
...
class RuleEngine:
    """Unified rule engine for user rules and dynamically discovered standards."""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize with configuration and discover standards."""
        self.config = config or {}
        self.rules = self.config.get('rules', {})
        self.analytics = self.config.get('analytics', {})
        self.filter_processor = FilterProcessor(config)
        
        # Get dynamic standard registry
        self.standard_registry = get_global_registry()
        
        # Merge discovered standards into rules
        self._merge_discovered_standards()
    ...
    def _merge_discovered_standards(self):
        """Merge dynamically discovered standards into rules."""
        discovered_rules = self.standard_registry.get_all_rules()
        
        # Add discovered rules to rules dict
        for rule_id, rule_config in discovered_rules.items():
            if rule_id not in self.rules:
                self.rules[rule_id] = rule_config
        
        logger.info(f"Loaded {len(discovered_rules)} standard rules dynamically")
    
    def get_available_standards(self) -> List[str]:
        """Get list of all available standard IDs."""
        return self.standard_registry.get_standard_ids()
    
    def get_rules_for_standard(self, standard_id: str) -> List[str]:
        """Get all rules for a specific standard."""
        return self.standard_registry.get_rules_for_standard(standard_id)
    ...
    def filter_rules(
        ...
        return self.standard_registry.filter_rules(
    ...
    def _evaluate_bidirectional_rule(
        ...
        """Evaluate bidirectional rules using library evaluator."""
        # Parse configuration using library function
        config = parse_bidirectional_config(rule_config)
        
        # Evaluate using library function
        return evaluate_bidirectional(data, config['min'], config['max'])
    ...
    def _evaluate_unidirectional_rule(
        ...
        """Evaluate unidirectional rules using library evaluator."""
        # Parse configuration using library function
        threshold = parse_unidirectional_config(rule_config)
        
        # Evaluate using library function
        if ascending:
            return evaluate_unidirectional_ascending(data, threshold)
        else:
            return evaluate_unidirectional_descending(data, threshold)
