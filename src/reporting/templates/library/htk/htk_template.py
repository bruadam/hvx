...
from src.reporting.templates.base_template import BaseTemplate
from src.reporting.charts.manager import get_chart_library_manager
from src.unified_analytics import UnifiedAnalyticsEngine, AnalysisType
from src.ventilation_rate_predictor import VentilationRatePredictor
...
class HTKReportTemplate(BaseTemplate):
    ...
    def _calculate_basic_statistics(self, mapped_data: Dict[str, pd.DataFrame], config: Optional[Dict[str, Any]] = None) -> Dict[str, Dict[str, Any]]:
        ...
        for room_id, df in mapped_data.items():
            ...
            if 'temperature' in df.columns:
                ...
                if len(temp_data) > 0:
                    room_stats['temperature'] = {
                        'mean': float(temp_data.mean()) if not pd.isna(temp_data.mean()) else 0.0,
                        'std': float(temp_data.std()) if not pd.isna(temp_data.std()) else 0.0,
                        'min': float(temp_data.min()) if not pd.isna(temp_data.min()) else 0.0,
                        'max': float(temp_data.max()) if not pd.isna(temp_data.max()) else 0.0,
                        'median': float(temp_data.median()) if not pd.isna(temp_data.median()) else 0.0,
                        'count': len(temp_data)
                    }
                else:
                    room_stats['temperature'] = {
            ...
            if 'co2' in df.columns:
                ...
                if len(co2_data) > 0:
                    room_stats['co2'] = {
                        'mean': float(co2_data.mean()) if not pd.isna(co2_data.mean()) else 0.0,
                        'std': float(co2_data.std()) if not pd.isna(co2_data.std()) else 0.0,
                        'min': float(co2_data.min()) if not pd.isna(co2_data.min()) else 0.0,
                        'max': float(co2_data.max()) if not pd.isna(co2_data.max()) else 0.0,
                        'median': float(co2_data.median()) if not pd.isna(co2_data.median()) else 0.0,
                        'count': len(co2_data)
                    }
                else:
                    room_stats['co2'] = {
            ...
            if 'humidity' in df.columns:
                ...
                if len(humidity_data) > 0:
                    room_stats['humidity'] = {
                        'mean': float(humidity_data.mean()) if not pd.isna(humidity_data.mean()) else 0.0,
                        'std': float(humidity_data.std()) if not pd.isna(humidity_data.std()) else 0.0,
                        'min': float(humidity_data.min()) if not pd.isna(humidity_data.min()) else 0.0,
                        'max': float(humidity_data.max()) if not pd.isna(humidity_data.max()) else 0.0,
                        'median': float(humidity_data.median()) if not pd.isna(humidity_data.median()) else 0.0,
    ...
    def _aggregate_building_statistics(self, rooms_with_charts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if temp_means:
            mean_val = np.mean(temp_means)
            building_stats['temperature']['mean'] = float(mean_val) if not pd.isna(mean_val) else 0.0
            
            temp_mins = [room['basic_statistics']['temperature']['min'] for room in rooms_with_stats if room['basic_statistics']['temperature']['count'] > 0]
            min_val = min(temp_mins) if temp_mins else 0.0
            building_stats['temperature']['min'] = float(min_val) if not pd.isna(min_val) else 0.0
            
            temp_maxs = [room['basic_statistics']['temperature']['max'] for room in rooms_with_stats if room['basic_statistics']['temperature']['count'] > 0]
            max_val = max(temp_maxs) if temp_maxs else 0.0
            building_stats['temperature']['max'] = float(max_val) if not pd.isna(max_val) else 0.0
            
            temp_medians = [room['basic_statistics']['temperature']['median'] for room in rooms_with_stats if room['basic_statistics']['temperature']['count'] > 0]
            median_val = np.median(temp_medians) if temp_medians else 0.0
            building_stats['temperature']['median'] = float(median_val) if not pd.isna(median_val) else 0.0
        ...
        if co2_means:
            mean_val = np.mean(co2_means)
            building_stats['co2']['mean'] = float(mean_val) if not pd.isna(mean_val) else 0.0
            
            co2_mins = [room['basic_statistics']['co2']['min'] for room in rooms_with_stats if room['basic_statistics']['co2']['count'] > 0]
            min_val = min(co2_mins) if co2_mins else 0.0
            building_stats['co2']['min'] = float(min_val) if not pd.isna(min_val) else 0.0
            
            co2_maxs = [room['basic_statistics']['co2']['max'] for room in rooms_with_stats if room['basic_statistics']['co2']['count'] > 0]
            max_val = max(co2_maxs) if co2_maxs else 0.0
            building_stats['co2']['max'] = float(max_val) if not pd.isna(max_val) else 0.0
            
            co2_medians = [room['basic_statistics']['co2']['median'] for room in rooms_with_stats if room['basic_statistics']['co2']['count'] > 0]
            median_val = np.median(co2_medians) if co2_medians else 0.0
            building_stats['co2']['median'] = float(median_val) if not pd.isna(median_val) else 0.0
        ...
        if humidity_means:
            mean_val = np.mean(humidity_means)
            building_stats['humidity']['mean'] = float(mean_val) if not pd.isna(mean_val) else 0.0
            
            humidity_mins = [room['basic_statistics']['humidity']['min'] for room in rooms_with_stats if room['basic_statistics']['humidity']['count'] > 0]
            min_val = min(humidity_mins) if humidity_mins else 0.0
            building_stats['humidity']['min'] = float(min_val) if not pd.isna(min_val) else 0.0
            
            humidity_maxs = [room['basic_statistics']['humidity']['max'] for room in rooms_with_stats if room['basic_statistics']['humidity']['count'] > 0]
            max_val = max(humidity_maxs) if humidity_maxs else 0.0
            building_stats['humidity']['max'] = float(max_val) if not pd.isna(max_val) else 0.0
            
            humidity_medians = [room['basic_statistics']['humidity']['median'] for room in rooms_with_stats if room['basic_statistics']['humidity']['count'] > 0]
            median_val = np.median(humidity_medians) if humidity_medians else 0.0
            building_stats['humidity']['median'] = float(median_val) if not pd.isna(median_val) else 0.0
    ...
    def _load_detailed_compliance_data(self, room_name: str) -> Dict[str, Any]:
        """Load detailed compliance data from compliance_details directory."""
        compliance_data = {}
        
        # Remove _processed suffix from room name for matching
        clean_room_name = room_name.replace("_processed", "")
        
        # Path to compliance details
        compliance_details_path = Path("output/analysis/compliance_details") / clean_room_name
        
        if not compliance_details_path.exists():
            logger.warning(f"No compliance details found for room: {room_name}")
            return compliance_data
        
        # Load all compliance files
        for compliance_dir in compliance_details_path.iterdir():
            if compliance_dir.is_dir():
                compliance_file = compliance_dir / f"{compliance_dir.name}_compliance.json"
                if compliance_file.exists():
                    try:
                        with open(compliance_file, 'r') as f:
                            ...
                            # Calculate mean from compliance data
                            mean_value = 0
                            try:
                                # Get all values from compliant and non-compliant records
                                all_records = data.get('compliant_records', []) + data.get('non_compliant_records', [])
                                if all_records:
                                    values = [record.get('value', 0) for record in all_records if record.get('value') is not None]
                                    if values:
                                        mean_value = sum(values) / len(values)
                            except Exception as e:
                                logger.debug(f"Could not calculate mean for {compliance_dir.name}: {e}")
                            
                            compliance_data[compliance_dir.name] = {
                                'compliance_rate': summary.get('compliance_rate', 0.0),
                                'total_records': summary.get('total_records', 0),
                                'compliant_records': summary.get('compliant_records', 0),
                                'non_compliant_records': summary.get('non_compliant_records', 0),
                                'mean': mean_value
                            }
                    except Exception as e:
                        logger.warning(f"Failed to load compliance data from {compliance_file}: {e}")
    ...
    def _prepare_report_data(self, analysis_data: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _process_building_data(self, building_name: str, building_data: Dict[str, Any], analysis_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if "rooms" in building_data:
            for room_name, room_data in building_data["rooms"].items():
                try:
                    ...
                    if room_info:  # Only add room if processing was successful
                        building_info["rooms"].append(room_info)
                        room_metrics.append(room_info["metrics"])
                except Exception as e:
                    logger.warning(f"Failed to process room {room_name} in building {building_name}: {e}")
                    continue
        
        # Sort rooms by performance (worst to best) and group by recommendations
        if building_info["rooms"]:
            building_info["rooms"] = self._sort_and_group_rooms_by_performance(building_info["rooms"])
        ...
        # Calculate temperature violation rates (expected by template)
        temp_above_26 = 0.0
        temp_above_27 = 0.0  
        temp_below_20 = 0.0
        
        if room_metrics:
            # Calculate average violation rates from room metrics
            temp_above_26_values = [r.get("temp_above_26_occupancy", 0) for r in room_metrics]
            temp_above_27_values = [r.get("temp_above_27_occupancy", 0) for r in room_metrics] 
            temp_below_20_values = [r.get("temp_below_20_occupancy", 0) for r in room_metrics]
            
            temp_above_26 = np.mean(temp_above_26_values) if temp_above_26_values else 0.0
            temp_above_27 = np.mean(temp_above_27_values) if temp_above_27_values else 0.0
            temp_below_20 = np.mean(temp_below_20_values) if temp_below_20_values else 0.0
        ...
        building_info.update({
            ...
            # Temperature violation rates (expected by template)
            "temp_above_26": temp_above_26,
            "temp_above_27": temp_above_27,
            "temp_below_20": temp_below_20,
            
            # CO2 compliance attributes expected by template
            "co2_compliance_1000": {
                "value": f"{co2_performance:.1f}",
                "class": self._get_performance_class(co2_performance)
            },
            "co2_compliance_2000": {
                "value": f"{building_info['metrics'].get('co2_2000_compliance', co2_performance):.1f}",
                "class": self._get_performance_class(building_info["metrics"].get("co2_2000_compliance", co2_performance))
            },
    ...
    def _process_room_data(self, room_name: str, room_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Process analysis data for a single room."""
        # Check if room has any valid data
        if not room_data or (isinstance(room_data, dict) and not room_data):
            logger.warning(f"No data found for room {room_name}")
            return None
        
        # Load detailed compliance data from compliance_details directory
        detailed_compliance = self._load_detailed_compliance_data(room_name)
        
        # Merge detailed compliance data into test_results
        if detailed_compliance:
            if "test_results" not in room_data:
                room_data["test_results"] = {}
            room_data["test_results"].update(detailed_compliance)
            
            # Debug: log what compliance data was loaded
            period_keys = [k for k in detailed_compliance.keys() if ('morning' in k or 'afternoon' in k or 'evening' in k)]
            logger.info(f"Loaded {len(period_keys)} period-specific compliance tests for {room_name}")
        
        # Check for indoor data or basic data structures (be more flexible)
        has_test_results = "test_results" in room_data and room_data["test_results"]
        has_statistics = "statistics" in room_data and room_data["statistics"]
        has_basic_stats = "basic_statistics" in room_data and room_data["basic_statistics"]
        
        # Be more permissive - allow rooms with minimal data
        if not (has_test_results or has_statistics or has_basic_stats):
            logger.debug(f"No detailed analysis data found for room {room_name} - creating with defaults")
            # Don't reject the room, just use defaults
        ...
        # Extract ventilation information
        room_info["ventilation"] = self._extract_ventilation_info(room_data)
        ...
        room_info["daily_periods"] = self._process_daily_periods(room_data, room_name)
        
        # Debug: Check what we got for daily periods
        if room_name == "reerslev_0.016_processed":
            print(f"\nDEBUG - Daily periods for {room_name}:")
            for period in room_info["daily_periods"]:
                print(f"  {period['name']}: CO2_1000={period['co2_1000_compliance']}, CO2_2000={period['co2_2000_compliance']}, Temp={period['temp_compliance']}")
        
        # Process seasonal data
        room_info["seasons"] = self._process_seasonal_data(room_data, room_name)
    ...
    def _sort_and_group_rooms_by_performance(self, rooms: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Sort rooms by performance (worst to best) and add grouping information."""
        # Calculate performance scores for each room
        for room in rooms:
            room["performance_score"] = self._calculate_room_performance_score(room)
            room["performance_category"] = self._get_performance_category(room["performance_score"])
            room["recommendation_category"] = self._categorize_room_recommendations(room)
        
        # Sort rooms by performance score (lowest/worst first)
        sorted_rooms = sorted(rooms, key=lambda r: r["performance_score"])
        
        # Add ranking information
        for i, room in enumerate(sorted_rooms):
            room["performance_rank"] = i + 1
            room["performance_rank_total"] = len(sorted_rooms)
        
        return sorted_rooms
    
    def _calculate_room_performance_score(self, room: Dict[str, Any]) -> float:
        """Calculate a combined performance score for a room (lower = worse performance)."""
        metrics = room.get("metrics", {})
        
        # CO2 performance (0-100, higher is better)
        co2_1000_compliance = metrics.get("co2_1000_compliance", 50.0)
        co2_2000_compliance = metrics.get("co2_2000_compliance", 80.0)
        
        # Temperature performance (0-100, higher is better)  
        temp_compliance = metrics.get("temp_compliance", 85.0)
        
        # Ventilation performance
        ventilation_info = room.get("ventilation", {})
        ach = ventilation_info.get("ach_mean", 1.0)
        # Convert ACH to score (2.0+ ACH = 100, 1.0 ACH = 50, 0.5 ACH = 25)
        ach_score = min(100, max(0, (ach / 2.0) * 100))
        
        # Weighted combination (CO2 40%, Temperature 35%, Ventilation 25%)
        performance_score = (co2_1000_compliance * 0.4 + 
                           temp_compliance * 0.35 + 
                           ach_score * 0.25)
        
        return performance_score
    
    def _get_performance_category(self, score: float) -> str:
        """Categorize room performance based on score."""
        if score >= 85:
            return "excellent"
        elif score >= 70:
            return "good" 
    ...
    def _categorize_room_recommendations(self, room: Dict[str, Any]) -> str:
        """Categorize room based on primary recommendation needed."""
        metrics = room.get("metrics", {})
        ventilation = room.get("ventilation", {})
        
        # Check for critical issues
        co2_1000 = metrics.get("co2_1000_compliance", 50.0)
        co2_2000 = metrics.get("co2_2000_compliance", 80.0) 
        temp_compliance = metrics.get("temp_compliance", 85.0)
        ach = ventilation.get("ach_mean", 1.0)
        
        # Priority: Critical CO2 > Poor ventilation > Temperature issues > Minor improvements
        if co2_2000 < 90:
            return "critical_co2"
        elif co2_1000 < 70 or ach < 1.5:
            return "ventilation_improvement"  
        elif temp_compliance < 75:
    ...
    def _extract_room_metrics(self, room_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract key metrics from room analysis data with occupancy-based temperature and CO2 metrics."""
        metrics = {
            # CO2 metrics during occupancy
            "co2_1000_compliance": 0,
            "co2_2000_compliance": 0,
            "avg_co2": 0,
            "co2_violations": 0,
            
            # Temperature metrics during occupancy  
            "temp_compliance": 0,
            "temp_above_26_occupancy": 0,
            "temp_above_27_occupancy": 0,
            "temp_below_20_occupancy": 0,
            "avg_temp": 0,
            "temp_violations": 0,
        ...
        if "test_results" in room_data:
            ...
            # CO2 1000ppm compliance (opening hours)
            co2_1000_test = test_results.get("co2_1000_all_year_opening", {})
            if "compliance_rate" in co2_1000_test:
                metrics["co2_1000_compliance"] = co2_1000_test["compliance_rate"]  # Already percentage
                metrics["co2_compliance"] = co2_1000_test["compliance_rate"]  # Legacy
            if "violations_count" in co2_1000_test:
                metrics["co2_violations"] = co2_1000_test["violations_count"]
            
            # CO2 2000ppm compliance (opening hours)
            co2_2000_test = test_results.get("co2_2000_all_year_opening", {})
            if "compliance_rate" in co2_2000_test:
                metrics["co2_2000_compliance"] = co2_2000_test["compliance_rate"]  # Already percentage
            else:
                # Fallback: try alternative test keys
                alternative_keys = ["co2_2000_opening", "co2_limit_2000_all_year_opening", "co2_limit_2000_opening"]
                for alt_key in alternative_keys:
                    alt_test = test_results.get(alt_key, {})
                    if "compliance_rate" in alt_test:
                        metrics["co2_2000_compliance"] = alt_test["compliance_rate"]
                        break
            ...
            # Temperature >26°C during occupancy
            temp_26_test = test_results.get("temp_above_26_all_year_opening", {})
            if "compliance_rate" in temp_26_test:
                metrics["temp_above_26_occupancy"] = 100 - temp_26_test["compliance_rate"]  # Convert to percentage above
            
            # Temperature >27°C during occupancy  
            temp_27_test = test_results.get("temp_above_27_all_year_opening", {})
            if "compliance_rate" in temp_27_test:
                metrics["temp_above_27_occupancy"] = 100 - temp_27_test["compliance_rate"]  # Convert to percentage above
            
            # Temperature <20°C during occupancy
            temp_20_test = test_results.get("temp_below_20_all_year_opening", {})
            if "compliance_rate" in temp_20_test:
                metrics["temp_below_20_occupancy"] = 100 - temp_20_test["compliance_rate"]  # Convert to percentage below
        ...
        # Fallback: Extract from compliance analysis if CO2 2000ppm compliance is still 0
        if "basic_statistics" in room_data and "compliance_analysis" in room_data["basic_statistics"]:
            compliance_analysis = room_data["basic_statistics"]["compliance_analysis"]
            co2_compliance = compliance_analysis.get("co2_compliance", {})
            opening_hours_co2 = co2_compliance.get("opening_hours", {})
            
            # Use compliance analysis for CO2 2000ppm if not found in tests
            if metrics["co2_2000_compliance"] == 0 and "below_2000_percentage" in opening_hours_co2:
                metrics["co2_2000_compliance"] = opening_hours_co2["below_2000_percentage"]
            
            # Use compliance analysis for CO2 1000ppm if not found in tests  
            if metrics["co2_1000_compliance"] == 0 and "below_1000_percentage" in opening_hours_co2:
                metrics["co2_1000_compliance"] = opening_hours_co2["below_1000_percentage"]
        
        return metrics
    
    def _extract_ventilation_info(self, room_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract comprehensive ventilation information from room data."""
        # Check if there's basic_statistics with ventilation data
        basic_stats = room_data.get("basic_statistics", {})
        if "ventilation" in basic_stats:
            ventilation_data = basic_stats["ventilation"]
            
            return {
                "ach_mean": ventilation_data.get("ach_mean", 0.0),
                "ach_std": ventilation_data.get("ach_std", 0.0), 
                "confidence": ventilation_data.get("confidence", "Low"),
                "confidence_score": ventilation_data.get("confidence_score", 0.0),
                "periods_analyzed": ventilation_data.get("periods_analyzed", 0),
                "ventilation_status": ventilation_data.get("ventilation_status", "Unknown"),
                "ventilation_grade": ventilation_data.get("ventilation_grade", "F"),
                "grade_description": ventilation_data.get("grade_description", ""),
                "recommendation": ventilation_data.get("recommendation", ""),
                "detailed_recommendation": ventilation_data.get("detailed_recommendation", ""),
                "efficiency_metrics": ventilation_data.get("efficiency_metrics", {}),
                "consistency_rating": ventilation_data.get("consistency_rating", "Unknown"),
                "air_renewal_efficiency": ventilation_data.get("air_renewal_efficiency", 0.0),
                "pollutant_removal_capacity": ventilation_data.get("pollutant_removal_capacity", 0.0),
                "occupancy_support_level": ventilation_data.get("occupancy_support_level", 0.0)
            }
        
        # Fallback: check older format or default values
        return {
            "ach_mean": 1.0,  # Default ventilation rate
            "ach_std": 0.5,
            "confidence": "Low",
            "confidence_score": 0.0,
            "periods_analyzed": 0,
            "ventilation_status": "Not calculated",
            "ventilation_grade": "F",
            "grade_description": "Analysis ikke tilgængelig",
            "recommendation": "Ventilation rate analysis not available for this room.",
            "detailed_recommendation": "Ventilationsanalyse er ikke tilgængelig for dette rum. Kontakt teknisk support for hjælp.",
            "efficiency_metrics": {},
            "consistency_rating": "Unknown",
            "air_renewal_efficiency": 0.0,
            "pollutant_removal_capacity": 0.0,
            "occupancy_support_level": 0.0
        }
    
    def _calculate_period_averages_from_timeseries(self, room_name: str, mapped_data_dir: Optional[str] = None) -> Dict[str, Dict[str, Optional[float]]]:
        """Calculate time-period-specific averages from raw timeseries data."""
        period_averages: Dict[str, Dict[str, Optional[float]]] = {
            "morning": {"avg_co2": None, "avg_temp": None},
            "afternoon": {"avg_co2": None, "avg_temp": None}, 
            "evening": {"avg_co2": None, "avg_temp": None}
        }
        ...
        try:
            ...
            if mapped_data_dir is None:
                mapped_data_dir = "output/mapped_data"
            
            # Clean room name for file lookup
            clean_room_name = room_name.replace('_processed', '')
            file_patterns = [
                f"{room_name}.csv",
                f"{clean_room_name}.csv",
                f"{room_name}_processed.csv"
            ]
            
            data_file = None
            for pattern in file_patterns:
                potential_file = Path(mapped_data_dir) / pattern
                if potential_file.exists():
                    data_file = potential_file
                    break
            
            if data_file is None:
                logger.warning(f"No timeseries data file found for room {room_name}")
            ...
            if 'timestamp' not in df.columns:
                logger.warning(f"No timestamp column in {data_file}")
                return period_averages
            ...
            for period_key, (start_hour, end_hour) in time_periods.items():
                period_mask = (df['hour'] >= start_hour) & (df['hour'] < end_hour)
                period_data = df[period_mask]
                
                if len(period_data) > 0:
                    # Calculate CO2 average
                    co2_columns = [col for col in df.columns if 'co2' in col.lower() and col != 'timestamp']
                    if co2_columns:
                        co2_values = period_data[co2_columns].mean(axis=1)
                        co2_values = co2_values.dropna()
                        if len(co2_values) > 0:
                            period_averages[period_key]["avg_co2"] = co2_values.mean()
                    
                    # Calculate Temperature average
                    temp_columns = [col for col in df.columns if 'temp' in col.lower() and col != 'timestamp']
                    if temp_columns:
                        temp_values = period_data[temp_columns].mean(axis=1)
                        temp_values = temp_values.dropna()
                        if len(temp_values) > 0:
                            period_averages[period_key]["avg_temp"] = temp_values.mean()
            
        except Exception as e:
            logger.warning(f"Error calculating period averages for {room_name}: {e}")
        
        return period_averages

    def _calculate_seasonal_averages_from_timeseries(self, room_name: str, mapped_data_dir: Optional[str] = None) -> Dict[str, Dict[str, Optional[float]]]:
        """Calculate season-specific averages from raw timeseries data."""
        seasonal_averages: Dict[str, Dict[str, Optional[float]]] = {
            "spring": {"avg_co2": None, "avg_temp": None},
            "summer": {"avg_co2": None, "avg_temp": None}, 
            "autumn": {"avg_co2": None, "avg_temp": None},
            "winter": {"avg_co2": None, "avg_temp": None}
        }
        ...
        try:
            ...
            for season_key, months in seasons.items():
                ...
                if len(season_data) > 0:
                    ...
                    co2_columns = [col for col in df.columns if 'co2' in col.lower() and col != 'timestamp']
                    if co2_columns:
                        co2_values = season_data[co2_columns].mean(axis=1)
                        co2_values = co2_values.dropna()
                        if len(co2_values) > 0:
                            seasonal_averages[season_key]["avg_co2"] = co2_values.mean()
                    
                    # Calculate Temperature average
                    temp_columns = [col for col in df.columns if 'temp' in col.lower() and col != 'timestamp']
                    if temp_columns:
                        temp_values = season_data[temp_columns].mean(axis=1)
                        temp_values = temp_values.dropna()
                        if len(temp_values) > 0:
                            seasonal_averages[season_key]["avg_temp"] = temp_values.mean()
            
        except Exception as e:
            logger.warning(f"Error calculating seasonal averages for {room_name}: {e}")
        
        return seasonal_averages

    def _process_daily_periods(self, room_data: Dict[str, Any], room_name: str) -> List[Dict[str, Any]]:
        ...
        # Get test_results if available, but don't return early if missing
        test_results = room_data.get("test_results", {})
        
        # Load compliance data for this room
        compliance_data = self._load_detailed_compliance_data(room_name)
        ...
        for period_key, period_name in period_names.items():
            ...
            # Look for period-specific test results with multiple key patterns
            co2_1000_test_keys = [
                f"co2_1000_{period_key}_opening",
                f"co2_1000_{period_key}",
                f"co2_limit_1000_{period_key}_opening",
                f"co2_limit_1000_{period_key}"
            ]
            co2_2000_test_keys = [
                f"co2_2000_{period_key}_opening",
                f"co2_2000_{period_key}",
                f"co2_limit_2000_{period_key}_opening",
                f"co2_limit_2000_{period_key}"
            ]
            temp_test_keys = [
                f"temp_comfort_zone_20_26_{period_key}_opening",
                f"temp_comfort_zone_{period_key}_opening",
                f"temperature_{period_key}_opening"
            ]
            
            # Try CO2 1000ppm compliance from compliance data first, then test_results
            co2_1000_found = False
            co2_1000_compliance_key = f"co2_1000_{period_key}_opening"
            if co2_1000_compliance_key in compliance_data:
                compliance_info = compliance_data[co2_1000_compliance_key]
                if isinstance(compliance_info, dict) and 'compliance_rate' in compliance_info:
                    compliance_rate = compliance_info['compliance_rate']
                    if isinstance(compliance_rate, (int, float)) and compliance_rate >= 0:
                        period_data["co2_1000_compliance"] = f"{compliance_rate:.1f}"
                        period_data["co2_1000_class"] = self._get_performance_class(compliance_rate)
                        co2_1000_found = True
                        print(f"DEBUG - Set co2_1000_compliance for {period_key}: {compliance_rate}")
            
            # Fallback to test_results if compliance data not found
            if not co2_1000_found:
                for co2_1000_test_key in co2_1000_test_keys:
                    if co2_1000_test_key in test_results and not co2_1000_found:
                        compliance_rate = test_results[co2_1000_test_key].get("compliance_rate", 0)
                        if compliance_rate >= 0:  # Show compliance rate even if it's 0%
                            period_data["co2_1000_compliance"] = f"{compliance_rate:.1f}"
                            period_data["co2_1000_class"] = self._get_performance_class(compliance_rate)
                            co2_1000_found = True
                        mean_value = test_results[co2_1000_test_key].get("mean", 0)
                        if mean_value > 0:
                            period_data["avg_co2"] = mean_value
                            period_data["avg_co2_class"] = "good" if mean_value < 800 else "warning" if mean_value < 1200 else "danger"
            
            # Try CO2 2000ppm compliance from compliance data first, then test_results
            co2_2000_found = False
            co2_2000_compliance_key = f"co2_2000_{period_key}_opening"
            if co2_2000_compliance_key in compliance_data:
                compliance_info = compliance_data[co2_2000_compliance_key]
                if isinstance(compliance_info, dict) and 'compliance_rate' in compliance_info:
                    compliance_rate = compliance_info['compliance_rate']
                    if isinstance(compliance_rate, (int, float)) and compliance_rate >= 0:
                        period_data["co2_2000_compliance"] = f"{compliance_rate:.1f}"
                        period_data["co2_2000_class"] = self._get_performance_class(compliance_rate)
                        co2_2000_found = True
                        print(f"DEBUG - Set co2_2000_compliance for {period_key}: {compliance_rate}")
            
            # Fallback to test_results if compliance data not found
            if not co2_2000_found:
                for co2_2000_test_key in co2_2000_test_keys:
                    if co2_2000_test_key in test_results and not co2_2000_found:
            ...
            # Fallback: Use all-year CO2 2000ppm data if period-specific not available
            if not co2_2000_found and "co2_2000_all_year_opening" in test_results:
                compliance_rate = test_results["co2_2000_all_year_opening"].get("compliance_rate", 0)
                if compliance_rate >= 0:
                    period_data["co2_2000_compliance"] = f"{compliance_rate:.1f}"
                    period_data["co2_2000_class"] = self._get_performance_class(compliance_rate)
            
            # Try temperature compliance from compliance data first, then test_results
            temp_found = False
            temp_compliance_key = f"temp_comfort_zone_20_26_{period_key}_opening"
            if temp_compliance_key in compliance_data:
                compliance_info = compliance_data[temp_compliance_key]
                if isinstance(compliance_info, dict) and 'compliance_rate' in compliance_info:
                    compliance_rate = compliance_info['compliance_rate']
                    if isinstance(compliance_rate, (int, float)) and compliance_rate >= 0:
                        ...
                        temp_found = True
                        print(f"DEBUG - Set temp_compliance for {period_key}: {compliance_rate}")
            
            # Fallback to test_results if compliance data not found
            if not temp_found:
                for temp_test_key in temp_test_keys:
                    if temp_test_key in test_results and not temp_found:
                        ...
                        if mean_value > 0:
                            period_data["avg_temp"] = mean_value
                            period_data["avg_temp_class"] = "good" if 20 <= mean_value <= 24 else "warning" if 18 <= mean_value <= 26 else "danger"
                            temp_found = True
            
            periods.append(period_data)
        
        # Enhanced fallback: Calculate period-specific averages from timeseries data
        # room_name is now passed as a parameter
        if room_name and any(period["avg_co2"] == "N/A" or period["avg_temp"] == "N/A" for period in periods):
            period_averages = self._calculate_period_averages_from_timeseries(room_name)
            
            # Apply the calculated period-specific averages
            for i, period in enumerate(periods):
                period_key = list({"morning": "Morgen (08-12)", "afternoon": "Eftermiddag (12-16)", "evening": "Aften (16-21)"}.keys())[i]
                
                # Update CO2 average if not already set and we have valid data
                if period["avg_co2"] == "N/A" and period_key in period_averages:
                    mean_co2 = period_averages[period_key]["avg_co2"]
                    if mean_co2 is not None and not pd.isna(mean_co2):
                        period["avg_co2"] = mean_co2
                        period["avg_co2_class"] = "good" if mean_co2 < 800 else "warning" if mean_co2 < 1200 else "danger"
                
                # Update temperature average if not already set and we have valid data
                if period["avg_temp"] == "N/A" and period_key in period_averages:
                    mean_temp = period_averages[period_key]["avg_temp"]
                    if mean_temp is not None and not pd.isna(mean_temp):
                        period["avg_temp"] = mean_temp
                        period["avg_temp_class"] = "good" if 20 <= mean_temp <= 24 else "warning" if 18 <= mean_temp <= 26 else "danger"
        
        # Final fallback: Use overall basic statistics if period-specific calculation failed
        for period_data in periods:
            if period_data["avg_co2"] == "N/A" and "basic_statistics" in room_data:
                basic_stats = room_data["basic_statistics"]
                if "co2" in basic_stats and basic_stats["co2"].get("mean", 0) > 0:
                    mean_co2 = basic_stats["co2"]["mean"]
                    period_data["avg_co2"] = mean_co2
                    period_data["avg_co2_class"] = "good" if mean_co2 < 800 else "warning" if mean_co2 < 1200 else "danger"
            
            if period_data["avg_temp"] == "N/A" and "basic_statistics" in room_data:
                basic_stats = room_data["basic_statistics"]
                if "temperature" in basic_stats and basic_stats["temperature"].get("mean", 0) > 0:
                    mean_temp = basic_stats["temperature"]["mean"]
                    period_data["avg_temp"] = mean_temp
                    period_data["avg_temp_class"] = "good" if 20 <= mean_temp <= 24 else "warning" if 18 <= mean_temp <= 26 else "danger"
        
        return periods
    
    def _process_seasonal_data(self, room_data: Dict[str, Any], room_name: str) -> List[Dict[str, Any]]:
        ...
        # Get test_results for seasonal compliance rates
        test_results = room_data.get("test_results", {})
        ...
        # Calculate seasonal averages from timeseries data
        # room_name is now passed as a parameter
        seasonal_averages = self._calculate_seasonal_averages_from_timeseries(room_name)
        
        for season_key, season_name in season_names.items():
            # Get seasonal averages for this season
            season_avg = seasonal_averages.get(season_key, {})
            ...
            season_data = {
                ...
                "co2_2000_compliance": "N/A", 
                "temp_compliance": "N/A",
                "avg_co2": season_avg.get("avg_co2", "N/A"),
                "avg_temp": season_avg.get("avg_temp", "N/A"),
                "co2_1000_class": "neutral",
                "co2_2000_class": "neutral",
                "temp_class": "neutral",
                "avg_co2_class": "neutral",
            ...
            # Set performance classes for averages
            if season_data["avg_co2"] != "N/A":
                avg_co2_val = float(season_data["avg_co2"])
                season_data["avg_co2_class"] = "good" if avg_co2_val < 800 else "warning" if avg_co2_val < 1200 else "danger"
            
            if season_data["avg_temp"] != "N/A":
                avg_temp_val = float(season_data["avg_temp"])
                season_data["avg_temp_class"] = "good" if 20 <= avg_temp_val <= 24 else "warning" if 18 <= avg_temp_val <= 26 else "danger"
            
            # Look for seasonal test results with multiple key patterns
            # Try different possible key formats for CO2 1000ppm
            co2_1000_test_keys = [
                f"co2_1000_{season_key}_opening",
                f"co2_1000_{season_key}",
                f"co2_limit_1000_{season_key}_opening",
                f"co2_limit_1000_{season_key}"
            ]
            co2_2000_test_keys = [
                f"co2_2000_{season_key}_opening", 
                f"co2_2000_{season_key}",
                f"co2_limit_2000_{season_key}_opening",
                f"co2_limit_2000_{season_key}"
            ]
            temp_test_keys = [
                f"temp_comfort_zone_20_26_{season_key}_opening",
                f"temp_comfort_zone_{season_key}_opening",
                f"temperature_{season_key}_opening"
            ]
            
            # Try CO2 1000ppm compliance from compliance data first, then test_results
            co2_1000_found = False
            co2_1000_compliance_key = f"co2_1000_{season_key}_opening"
            if co2_1000_compliance_key in compliance_data:
                compliance_info = compliance_data[co2_1000_compliance_key]
                if isinstance(compliance_info, dict) and 'compliance_rate' in compliance_info:
                    compliance_rate = compliance_info['compliance_rate']
                    if isinstance(compliance_rate, (int, float)) and compliance_rate >= 0:
                        ...
                        co2_1000_found = True
                        print(f"DEBUG - Set seasonal co2_1000_compliance for {season_key}: {compliance_rate}")
            ...
            if season_data["avg_co2"] == "N/A" and opening_hours_co2.get('mean_co2') is not None:
                try:
                    mean_co2 = float(opening_hours_co2['mean_co2'])
                    season_data["avg_co2"] = mean_co2
                    season_data["avg_co2_class"] = "good" if mean_co2 < 800 else "warning" if mean_co2 < 1200 else "danger"
                except (ValueError, TypeError):
                    pass
            
            if season_data["avg_temp"] == "N/A" and opening_hours_temp.get('mean_temperature') is not None:
                try:
                    mean_temp = float(opening_hours_temp['mean_temperature'])
                    season_data["avg_temp"] = mean_temp
            ...
            # Try CO2 2000ppm compliance from compliance data first, then test_results
            co2_2000_found = False
            co2_2000_compliance_key = f"co2_2000_{season_key}_opening"
            if co2_2000_compliance_key in compliance_data:
                compliance_info = compliance_data[co2_2000_compliance_key]
                if isinstance(compliance_info, dict) and 'compliance_rate' in compliance_info:
                    compliance_rate = compliance_info['compliance_rate']
                    if isinstance(compliance_rate, (int, float)) and compliance_rate >= 0:
                        ...
                        co2_2000_found = True
                        print(f"DEBUG - Set seasonal co2_2000_compliance for {season_key}: {compliance_rate}")
            ...
            # Try temperature compliance from compliance data first, then test_results
            temp_found = False
            temp_compliance_key = f"temp_comfort_zone_20_26_{season_key}_opening"
            if temp_compliance_key in compliance_data:
                compliance_info = compliance_data[temp_compliance_key]
                if isinstance(compliance_info, dict) and 'compliance_rate' in compliance_info:
                    compliance_rate = compliance_info['compliance_rate']
                    if isinstance(compliance_rate, (int, float)) and compliance_rate >= 0:
                        ...
                        temp_found = True
                        print(f"DEBUG - Set seasonal temp_compliance for {season_key}: {compliance_rate}")
            ...
            # Fallback to all_year temperature test if season-specific not available
            if not temp_found:
                all_year_temp_keys = [
                    "temp_comfort_zone_20_26_all_year_opening",
                    "temp_comfort_zone_all_year_opening", 
                    "temperature_all_year_opening",
                    "temp_comfort_zone_20_26_opening",
                    "temperature_opening"
                ]
                for all_year_temp_key in all_year_temp_keys:
                    if all_year_temp_key in test_results:
                        compliance_rate = test_results[all_year_temp_key].get("compliance_rate", 0)
                        if compliance_rate >= 0:  # Show compliance rate even if it's 0%
                            season_data["temp_compliance"] = f"{compliance_rate:.1f}"
                            season_data["temp_class"] = self._get_performance_class(compliance_rate)
                        mean_value = test_results[all_year_temp_key].get("mean", 0)
                        if mean_value > 0:
                            season_data["avg_temp"] = f"{mean_value:.1f}"
                            season_data["avg_temp_class"] = "good" if 20 <= mean_value <= 24 else "warning" if 18 <= mean_value <= 26 else "danger"
                        break
            
            # Fallback: Keep N/A if timeseries data not available
            # Note: We intentionally avoid using overall basic_statistics here
            # as they don't represent seasonal-specific data
            if season_data["avg_co2"] == "N/A":
                # Could add logging here to indicate missing seasonal data
                pass
            ...
            # Always include the season data, even if we couldn't find specific test results
            # This ensures the table shows all seasons with "N/A" for missing data
    ...
    def _calculate_overall_metrics(self, building_metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Calculate overall temperature violation rates
        overall_temp_above_26 = np.mean([b.get("temp_above_26", 0) for b in building_metrics])
        overall_temp_above_27 = np.mean([b.get("temp_above_27", 0) for b in building_metrics])
        overall_temp_below_20 = np.mean([b.get("temp_below_20", 0) for b in building_metrics])
        
        # Handle NaN values
        overall_performance = 0.0 if pd.isna(overall_performance) else overall_performance
        overall_co2 = 0.0 if pd.isna(overall_co2) else overall_co2
        overall_temp = 0.0 if pd.isna(overall_temp) else overall_temp
        overall_temp_above_26 = 0.0 if pd.isna(overall_temp_above_26) else overall_temp_above_26
        overall_temp_above_27 = 0.0 if pd.isna(overall_temp_above_27) else overall_temp_above_27
        overall_temp_below_20 = 0.0 if pd.isna(overall_temp_below_20) else overall_temp_below_20
        ...
        return {
            ...
            # Temperature violation rates
            "temp_above_26": round(float(overall_temp_above_26), 1),
            "temp_above_27": round(float(overall_temp_above_27), 1),
            "temp_below_20": round(float(overall_temp_below_20), 1)
        }
    ...
    def _get_room_display_name(self, room_name: str, building_name: str) -> str:
        """Extract room identifier without building name for display purposes."""
        # Remove building name from room name for cleaner display
        building_variations = [
            building_name.lower(),
            building_name.lower().replace('_', '-'),
            building_name.lower().replace('-', '_'),
            building_name.lower().replace(' ', '_'),
            building_name.lower().replace(' ', '-'),
        ]
        
        clean_room_name = room_name.lower()
        
        # Remove building name prefixes
        for variation in building_variations:
            if clean_room_name.startswith(variation + '_'):
                clean_room_name = clean_room_name[len(variation) + 1:]
                break
            elif clean_room_name.startswith(variation + '-'):
                clean_room_name = clean_room_name[len(variation) + 1:]
                break
        
        # Remove _processed suffix
        clean_room_name = clean_room_name.replace('_processed', '').replace('-processed', '')
        
        # Convert to friendly display format
        return self._get_friendly_name(clean_room_name)
    ...
    def _calculate_ach_analysis(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Enhanced ventilation classification and detailed recommendations
            ventilation_grade = self._classify_ventilation_grade(float(ach_mean), confidence)
            detailed_recommendation = self._generate_detailed_ventilation_recommendations(float(ach_mean), float(ach_std), confidence, n_periods)
            
            # Calculate additional ventilation efficiency metrics
            efficiency_metrics = self._calculate_ventilation_efficiency_metrics(df, float(ach_mean))
            ...
            return {
                ...
                'confidence_score': round(confidence_prob, 2),
                'periods_analyzed': n_periods,
                'total_data_points': total_points,
                'ventilation_status': status,
                'ventilation_grade': ventilation_grade,
                'grade_description': self._get_grade_description(ventilation_grade),
                'recommendation': recommendation,
                'detailed_recommendation': detailed_recommendation,
                'efficiency_metrics': efficiency_metrics,
                'air_renewal_efficiency': efficiency_metrics['efficiency'],
                'pollutant_removal_capacity': efficiency_metrics['removal_capacity'],
                'occupancy_support_level': efficiency_metrics['occupancy_support'],
                'consistency_rating': self._calculate_ventilation_consistency(ach_values),
                'performance_category': self._get_ventilation_performance_category(float(ach_mean), float(ach_std), confidence)
            }
    ...
    def _get_grade_description(self, grade: str) -> str:
        """Get description for ventilation grade."""
        descriptions = {
            'A': 'Fremragende ventilation - overgår anbefalinger betydeligt',
            'B': 'Meget god ventilation - god luftskiftefrekvens', 
            'C': 'God ventilation - overholder grundlæggende krav',
            'D': 'Acceptabel ventilation - marginalt tilfredsstillende',
            'E': 'Dårlig ventilation - under acceptable grænser',
            'F': 'Kritisk dårlig ventilation - øjeblikkelig handling påkrævet'
        }
        return descriptions.get(grade, 'Ukendt ventilationsstatus')
    
    def _classify_ventilation_grade(self, ach_mean: float, confidence: str) -> str:
        """Classify ventilation performance with letter grades (A-F)."""
        # Adjust for confidence level
        confidence_factor = 1.0 if confidence == 'High' else 0.9 if confidence == 'Medium' else 0.8
        adjusted_ach = ach_mean * confidence_factor
        
        if adjusted_ach >= 4.0:
            return 'A'  # Excellent
        elif adjusted_ach >= 3.0:
            return 'B'  # Very Good
        elif adjusted_ach >= 2.5:
            return 'C'  # Good
        elif adjusted_ach >= 2.0:
            return 'D'  # Adequate
        elif adjusted_ach >= 1.5:
            return 'E'  # Poor
        else:
            return 'F'  # Critical
    
    def _generate_detailed_ventilation_recommendations(self, ach_mean: float, ach_std: float, confidence: str, n_periods: int) -> str:
        """Generate comprehensive ventilation recommendations."""
        recommendations = []
        
        # Performance-based recommendations
        if ach_mean < 1.0:
            recommendations.append("🚨 KRITISK: Ventilationen er farligt lav - øjeblikkelig handling påkrævet")
            recommendations.append("💨 Overvej mekanisk ventilation eller øget naturlig ventilation")
        elif ach_mean < 1.5:
            recommendations.append("⚠️ DÅRLIG: Ventilationsraten skal øges betydeligt")
            recommendations.append("🔧 Tjek HVAC-system og justér indstillinger")
        elif ach_mean < 2.0:
            recommendations.append("📈 UNDER STANDARD: Øg ventilationsraten til minimum 2.0 ACH")
            recommendations.append("🔍 Verificer ventilationssystemets drift og kapacitet")
        elif ach_mean < 2.5:
            recommendations.append("✅ ACCEPTABEL: Opfylder minimumskrav men kan forbedres")
        elif ach_mean < 3.0:
            recommendations.append("👍 GOD: Ventilationsraten er over minimum og tilfredsstillende")
        else:
            recommendations.append("🌟 FREMRAGENDE: Ventilationsraten er optimal")
        
        # Consistency recommendations
        if ach_std > 1.0:
            recommendations.append("📊 Høj variation i ventilation - undersøg systemets konsistens")
            recommendations.append("⚙️ Overvej konstant volumen ventilation for bedre stabilitet")
        
        # Data quality recommendations
        if confidence == 'Low':
            recommendations.append(f"📉 Lav pålidelighedsgrad ({n_periods} perioder analyseret)")
            recommendations.append("📅 Overvej længere overvågningsperiode for bedre vurdering")
        elif n_periods < 5:
            recommendations.append("⏱️ Få analyserede perioder - længere dataindsamling anbefales")
        
        # Efficiency recommendations
        if ach_mean > 0:
            if ach_mean < 2.0:
                recommendations.append("🌬️ Overvej behovsstyret ventilation for energioptimering")
            elif ach_mean > 4.0:
                recommendations.append("💡 Høj ventilationsrate - verificer energiforbrug og optimer om nødvendigt")
        
        return " • ".join(recommendations[:4])  # Limit to 4 key recommendations
    
    def _calculate_ventilation_efficiency_metrics(self, df: pd.DataFrame, ach_mean: float) -> Dict[str, float]:
        """Calculate additional ventilation efficiency metrics."""
        try:
            efficiency_metrics = {
                'efficiency': 0.0,
                'removal_capacity': 0.0,
                'occupancy_support': 0
            }
            
            if 'co2' in df.columns and ach_mean > 0:
                co2_data = df['co2'].dropna()
                if len(co2_data) > 0:
                    # Calculate ventilation efficiency (0-100%)
                    # Based on ACH relative to optimal range (2.5-3.5 ACH)
                    optimal_ach = 3.0
                    if ach_mean <= optimal_ach:
                        efficiency = (ach_mean / optimal_ach) * 100
                    else:
                        # Penalize excessive ventilation
                        excess_factor = min(2.0, ach_mean / optimal_ach)
                        efficiency = 100 / excess_factor
                    
                    efficiency_metrics['efficiency'] = round(min(100, max(0, efficiency)), 1)
                    
                    # Calculate CO2 removal capacity (ppm/hour)
                    # Simplified model: ACH * volume-based removal factor
                    base_removal = 300  # Base CO2 removal per ACH
                    removal_capacity = ach_mean * base_removal
                    efficiency_metrics['removal_capacity'] = round(removal_capacity, 0)
                    
                    # Estimate maximum occupancy support
                    # Based on CO2 generation per person (≈40 L/h) and ventilation capacity
                    co2_per_person_lph = 40  # L/h
                    room_volume_estimate = 100  # m³ (simplified assumption)
                    fresh_air_rate = ach_mean * room_volume_estimate * 1000  # L/h
                    max_occupancy = max(1, int(fresh_air_rate / (co2_per_person_lph * 20)))  # Factor of 20 for mixing
                    efficiency_metrics['occupancy_support'] = min(20, max_occupancy)  # Cap at 20 people
            
            return efficiency_metrics
            
        except Exception:
            return {'efficiency': 0.0, 'removal_capacity': 0.0, 'occupancy_support': 0}
    
    def _calculate_ventilation_consistency(self, ach_values: list) -> str:
        """Calculate consistency rating for ventilation performance."""
        if len(ach_values) < 2:
            return "Insufficient data"
        
        cv = np.std(ach_values) / np.mean(ach_values) if np.mean(ach_values) > 0 else float('inf')
    ...
    def _get_ventilation_performance_category(self, ach_mean: float, ach_std: float, confidence: str) -> str:
        """Get overall performance category for ventilation."""
        grade = self._classify_ventilation_grade(ach_mean, confidence)
        consistency = self._calculate_ventilation_consistency([ach_mean - ach_std, ach_mean, ach_mean + ach_std])
        
        if grade in ['A', 'B'] and consistency in ['Very Consistent', 'Consistent'] and confidence == 'High':
            return "Excellent"
        elif grade in ['B', 'C'] and consistency not in ['Extremely Variable'] and confidence in ['High', 'Medium']:
            return "Good"
        elif grade in ['C', 'D'] and confidence in ['High', 'Medium']:
            return "Adequate"
        elif grade in ['D', 'E']:
            return "Poor"
        else:
    ...
    def _generate_building_summary(self, building_name: str, basic_statistics: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if temp_issues:
            if any('over 27°C' in issue for issue in temp_issues):
                ...
                building_summary['recommendations'].append("⚠️ Medium prioritet: Overvej solafskærmning og ventilationsoptimering")
            if any('under 20°C' in issue for issue in temp_issues):
                building_summary['recommendations'].append("🌡️ Juster varmeregulering for at undgå energispild og komfortproblemer")
        
        # Enhanced ventilation-based recommendations
        if ventilation_issues:
            # Analyze ventilation grades across the building
            ventilation_grades = []
            critical_rooms = []
            poor_rooms = []
            ...
            for room_id, room_stats in building_rooms.items():
                room_name = self._get_friendly_name(room_id)
                ventilation = room_stats.get('ventilation', {})
                grade = ventilation.get('ventilation_grade', 'F')
                ach_mean = ventilation.get('ach_mean', 0.0)
                
                ventilation_grades.append(grade)
                
                if grade in ['F', 'E'] or ach_mean < 1.0:
                    critical_rooms.append(f"{room_name} (Karakter {grade}, {ach_mean:.2f} ACH)")
                elif grade in ['D', 'C']:
                    poor_rooms.append(f"{room_name} (Karakter {grade}, {ach_mean:.2f} ACH)")
            
            # Grade-based recommendations
            grade_counts = {grade: ventilation_grades.count(grade) for grade in ['A', 'B', 'C', 'D', 'E', 'F']}
            
            if grade_counts['F'] + grade_counts['E'] > 0:
                building_summary['recommendations'].append(f"🚨 KRITISK: {grade_counts['F'] + grade_counts['E']} rum har kritisk dårlig ventilation - øjeblikkelig handling påkrævet")
                building_summary['recommendations'].append("💨 Overvej installation af mekanisk ventilation eller forbedring af naturlig ventilation")
            
            if grade_counts['D'] + grade_counts['C'] > 0:
                building_summary['recommendations'].append(f"⚠️ DÅRLIG: {grade_counts['D'] + grade_counts['C']} rum har utilstrækkelig ventilation")
                building_summary['recommendations'].append("🔧 Tjek og optimer HVAC-system indstillinger")
            
            if critical_rooms:
                building_summary['recommendations'].append(f"🎯 Fokusér først på: {', '.join(critical_rooms[:3])}")
            
            # Ventilation system recommendations
            building_summary['recommendations'].append("📊 Implementer kontinuerlig overvågning af ventilationseffektivitet")
            building_summary['recommendations'].append("🔄 Overvej behovsstyret ventilation for bedre energieffektivitet og luftkvalitet")
        
        if co2_issues:
            building_summary['recommendations'].append("🌬️ Optimer ventilationssystem og kontroller luftskiftefrekvens")
            building_summary['recommendations'].append("📈 Overvej CO₂-sensorstyret ventilation i rum med høje koncentrationer")
        ...
        if len(building_summary['recommendations']) == 0:
            ...
            building_summary['recommendations'].append("📊 Overvej implementering af avanceret ventilationsovervågning for fremtidig optimering")
    ...
    def _prepare_template_context(self, report_data: Dict[str, Any], chart_paths: Dict[str, str], mapped_data: Optional[Dict[str, pd.DataFrame]] = None, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for building in report_data["buildings"]:
            ...
            for room in building["rooms"]:
                # Clean room name for chart path (remove _processed suffix)
                clean_room_name = room['name'].replace('_processed', '')
                # Room ID should just be the clean room name since charts are already named with full path
                room_id = clean_room_name.lower().replace(' ', '_')
                full_room_id = f"{building_id}_{room['name']}"
                
                # Add friendly name for room
                room_friendly_name = self._get_friendly_name(room["name"])
                room_display_name = self._get_room_display_name(room["name"], building["name"])
                
                # Create charts object for this room
                room_charts = {
                    "time_series_analysis": relative_chart_paths.get(f'yearly_trends_{room_id}', ''),
                    ...
                    "detailed_compliance": relative_chart_paths.get(f'detailed_compliance_{room_id}', '')
                }
                ...
                room_with_charts["display_name"] = room_display_name
                
                # Add separated CO2 compliance metrics
                room_metrics = room.get("metrics", {})
                room_with_charts["co2_1000_compliance"] = room_metrics.get("co2_1000_compliance", room.get("co2_performance", {}).get("value", 0))
                room_with_charts["co2_2000_compliance"] = room_metrics.get("co2_2000_compliance", 0)
                ...
                for room_id_attempt in possible_room_ids:
                    if room_id_attempt in basic_statistics:
                        room_with_charts["basic_statistics"] = self._make_json_serializable(basic_statistics[room_id_attempt])
                ...
                # Recalculate daily_periods and seasons with basic_statistics for proper averages
                if "basic_statistics" in room_with_charts:
                    room_with_charts["daily_periods"] = self._process_daily_periods(room_with_charts, room["name"])
                    room_with_charts["seasons"] = self._process_seasonal_data(room_with_charts, room["name"])
            ...
            if rooms_with_charts:
                building_with_charts["basic_statistics"] = self._make_json_serializable(self._aggregate_building_statistics(rooms_with_charts))
        ...
        # Save the context in json
        context_file = "output/report_context.json"
    ...
    def _make_json_serializable(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            # Skip DataFrames entirely - they shouldn't be in the context
    ...
    def _generate_executive_summary_text(self, report_data: Dict[str, Any]) -> str:
        ...
        til forbedringer baseret på etablerede standarder og best practices.co
        """
    ...
    def _calculate_building_data_quality(self, rooms: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Round the average completeness
        avg_completeness = round(avg_completeness, 1)

        # Determine missing periods and quality score based on average completeness
        ...
        try:
            ...
            print_css = CSS(string='''
                ...
                * {
                ...
                .metric-card, .building-summary, .room {
                    break-inside: avoid;
                    page-break-inside: avoid;
                }
            ...
            html_doc.write_pdf(
                ...
                font_config=font_config,
                presentational_hints=True,  # Enable CSS optimization
